...
from ray_utilities.constants import DEFAULT_VIDEO_KEYS, EPISODE_BEST_VIDEO, EPISODE_WORST_VIDEO
...
def remove_videos(
    ...
    did_copy = False
    for keys in DEFAULT_VIDEO_KEYS:
        subdir = metrics
        for key in keys[:-1]:
    ...
    for keys in DEFAULT_VIDEO_KEYS:
        ...
        for key in keys[:-1]:
            ...
            if (
                keys[-1] in subdir
                and "video"
                in subdir[keys[-1]]  # pyright: ignore[reportGeneralTypeIssues, reportTypedDictNotRequiredAccess]
            ):
                if not did_copy:
                    metrics = metrics.copy()
                    did_copy = True
